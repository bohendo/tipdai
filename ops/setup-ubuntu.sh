...
network="${2:-rinkeby}"
user="tipdai"
mnemonic_name="tipdai_mnemonic_$network" # name of docker secret to store private mnemonic in
prvkey="$HOME/.ssh/`whoami`"

# Sanity checks
if [[ -z "$1" ]]
then echo "Provide the server's hostname or ip address as the first ($1) arg" && exit
fi
...
# Prepare to load the hub's mnemonic into the server's secret store
echo "Copy the $mnemonic_name secret to your clipboard then paste it below & hit enter (no echo)"
echo -n "> "
read -s mnemonic
echo
...
then
  ...
  echo
  echo "Enter sudo password for REMOTE machine (no echo)"
  echo -n "> "
  read -s password
  echo
fi
...
# Setup docker secret
if [[ -n "\`docker secret ls | grep "$mnemonic_name"\`" ]]
then echo "A secret called $mnemonic_name already exists, aborting secret setup"
else
  id="\`echo $mnemonic | tr -d ' \n\r' | docker secret create $mnemonic_name -\`"
  if [[ "$?" == "0" ]]
  then
    echo "Successfully loaded mnemonic into secret store"
    echo "name=$mnemonic_name id=\$id"
    echo
  else echo "Something went wrong creating secret called $mnemonic_name"
  fi
fi
